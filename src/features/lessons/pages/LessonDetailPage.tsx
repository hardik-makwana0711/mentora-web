...
import { createMentorNote, fetchSessionDetail, updateMentorNote } from '@/services/lessons.service';
...
import { completeSession, fetchTranscriptStatus } from '@/services/lessons.service';
...
export default function LessonDetailPage() {
  ...
  const showRecordingNotice =
    d.status === 'scheduled' || d.status === 'rescheduled' || d.status === 'in_progress';
  ...
  return (
    <PageContainer>
      ...
      <PageHeader
        title={tr.lessonDetailTitle}
        description={formatLessonServiceName(d.subject_name)}
      />
      ...
      <div className="rounded-2xl border border-[var(--color-m-card-border)] bg-[var(--color-m-card)] p-5">
        ...
        <p className="mt-4 text-sm text-[var(--color-text-muted)]">
          {formatLessonDateTime(d.scheduled_at)}
        </p>
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          ...
          <Button
            ...
            onClick={() => {
              ...
              params.set('lessonId', d.lesson_id);
              params.set('studentId', d.student_id);
              params.set('mentorId', d.mentor_id);
              if (user?.role === 'parent' && user.id) params.set('parentId', user.id);
